...
* Provides `PluginLauncher` Ignition plugin that launches any nested child Ignition plugins. Useful when adding both Ignition and Gazebo plugins to the same sdf file, so Gazebo will not search for and inadvertently load the Ignition plugins.
